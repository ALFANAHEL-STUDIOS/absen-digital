...
export default function StudentReport() {
 ...
 // New states for month/year filtering
 const [selectedMonth, setSelectedMonth] = useState(format(new Date(), "MM"));
 const [selectedYear, setSelectedYear] = useState(format(new Date(), "yyyy"));
 const [currentMonthDisplay, setCurrentMonthDisplay] = useState(format(new Date(), "MMMM yyyy", { locale: id }));
 ...
 // Update current month display when month/year changes
 useEffect(() => {
   const monthIndex = parseInt(selectedMonth) - 1;
   const date = new Date(parseInt(selectedYear), monthIndex, 1);
   setCurrentMonthDisplay(format(date, "MMMM yyyy", { locale: id }));

   // Update date range for the selected month/year
   const startOfMonth = new Date(parseInt(selectedYear), monthIndex, 1);
   const endOfMonth = new Date(parseInt(selectedYear), monthIndex + 1, 0);
 ...
 useEffect(() => {
   ...
   if (userRole === 'student' && userData) {
     ...
     fetchStudentAttendanceData(studentData.id, selectedMonth, selectedYear);
   } else {
 ...
 const fetchStudentAttendanceData = async (studentId: string, month?: string, year?: string) => {
   ...
   try {
     ...
     // Use provided month/year or current selected values
     const targetMonth = month || selectedMonth;
     const targetYear = year || selectedYear;

     // Create date range for the selected month/year
     const startOfMonth = `${targetYear}-${targetMonth.padStart(2, '0')}-01`;
     const endOfMonth = `${targetYear}-${targetMonth.padStart(2, '0')}-31`;
     ...
     if (records.length > 0) {
       ...
       chartData[monthIndex].hadir = present;
       chartData[monthIndex].sakit = sick;
       chartData[monthIndex].izin = permitted;
       chartData[monthIndex].alpha = absent;
     }
 ...
 useEffect(() => {
   ...
   if (selectedStudent) {
     fetchStudentAttendanceData(selectedStudent.id, selectedMonth, selectedYear);
 ...
 }, [selectedStudent, schoolId, selectedMonth, selectedYear]);

 const handleStudentSelect = (student: any) => {
   setSelectedStudent(student);
   fetchStudentAttendanceData(student.id, selectedMonth, selectedYear);
 };
 ...
 const handleMonthChange = (month: string) => {
   ...
   if (selectedStudent) {
     fetchStudentAttendanceData(selectedStudent.id, month, selectedYear);
   }
 };
 ...
 const handleYearChange = (year: string) => {
   ...
   if (selectedStudent) {
     fetchStudentAttendanceData(selectedStudent.id, selectedMonth, year);
   }
 };
 ...
 const handleDownloadPDF = async () => {
   ...
   try {
     ...
     doc.setFontSize(13);
     doc.text(`BULAN ${currentMonthDisplay.toUpperCase()}`, pageWidth / 2, margin + 38, { align: "center" });
     ...
     // Save the PDF
     const fileName = `Rekap_Siswa_${selectedStudent?.name || "Unknown"}_${selectedMonth}_${selectedYear}.pdf`;
     doc.save(fileName);
 ...
 const handleDownloadExcel = async () => {
   ...
   try {
     ...
     const fileName = `Laporan_${studentName}_${selectedMonth}_${selectedYear}.xlsx`;
           ...
           <div className="space-y-6">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
               <button
                 ...
                 className="flex items-center justify-center gap-3 bg-red-600 text-white p-4 rounded-xl hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
               >
                 {isDownloading ? (
               ...
               <button
                 ...
                 disabled={isDownloading || loading}
                 className="flex items-center justify-center gap-3 bg-green-600 text-white p-4 rounded-xl hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
               >
                 {isDownloading ? (
                   <Loader2 className="h-6 w-6 animate-spin" />
                 ) : (
