...
const handleDownloadPDF = async () => {
  ...
  try {
    ...
    const margin = 15;
    const footerHeight = 50; // Dikurangi dari 60 ke 50 untuk memberikan lebih banyak ruang tabel
    ...
    const addPageHeader = (isFirstPage = false) => {
      ...
      pdfDoc.setFont("helvetica", "bold");
      pdfDoc.text(schoolInfo.address, pageWidth / 2, margin + 6, { align: "center" }); // Dikurangi dari 7 ke 6
      pdfDoc.text(`NPSN ${schoolInfo.npsn}`, pageWidth / 2, margin + 12, { align: "center" }); // Dikurangi dari 14 ke 12
      // Add horizontal line
      pdfDoc.setLineWidth(0.5);
      pdfDoc.line(margin, margin + 16, pageWidth - margin, margin + 16); // Dikurangi dari 20 ke 16
      if (isFirstPage) {
        // Add title only on first page
        pdfDoc.setFontSize(12);
        pdfDoc.setFont("helvetica", "normal");
        pdfDoc.text("REKAPITULASI LAPORAN ABSENSI GURU", pageWidth / 2, margin + 24, { align: "center" }); // Dikurangi dari 30 ke 24
        pdfDoc.text(`BULAN ${formattedMonth.toUpperCase()}`, pageWidth / 2, margin + 30, { align: "center" }); // Dikurangi dari 36 ke 30
        return margin + 36; // Dikurangi dari 43 ke 36
      } else {
        // Smaller header for continuation pages
        pdfDoc.setFontSize(10);
        pdfDoc.setFont("helvetica", "normal");
        return margin + 24; // Dikurangi dari 32 ke 24
      }
    };
    ...
    yPos += 8; // Ditambah dari 18 ke 25 untuk memberikan jarak lebih
    ...
    // Add signature section dengan jarak yang lebih tinggi
    yPos += 15; // Ditambah jarak antara akhir tabel dengan tanda tangan
