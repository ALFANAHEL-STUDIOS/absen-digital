...
export const studentApi = {
 // Create a new student with class validation
 create: async (schoolId: string, studentData: any) => {
   try {
     // Validate that the class exists
     if (studentData.class) {
       const classes = await ClassService.search(schoolId, 'name', '==', studentData.class);
       if (classes.length === 0) {
         throw new Error('Kelas yang dipilih tidak valid');
       }
     }
     const result = await StudentService.create(schoolId, {
       ...studentData,
       createdAt: new Date(),
       updatedAt: new Date()
     });
 ...
 // Update student information with class validation
 update: async (schoolId: string, studentId: string, data: any) => {
   try {
     // Validate that the class exists if class is being updated
     if (data.class) {
       const classes = await ClassService.search(schoolId, 'name', '==', data.class);
       if (classes.length === 0) {
         throw new Error('Kelas yang dipilih tidak valid');
       }
     }
     await StudentService.update(schoolId, studentId, {
       ...data,
       updatedAt: new Date()
     });
